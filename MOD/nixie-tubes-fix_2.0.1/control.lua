...
local function displayValString(entity,vs,color)
    ...
    while entity do
        ...
        if ( chcount < 3 ) then
            ...
            elseif offset >= chcount then
                if chcount == 1 then
                    setStates( entity, cache, { vs:sub( offset, offset ) }, color )
                else
                    setStates( entity, cache, { vs:sub( offset - 1, offset - 1 ), vs:sub( offset, offset ) }, color )
                end
            end
        elseif ( chcount == 3 ) then
            if not vs then
                setStates( entity, cache, { "off", "off", "off" } )
            elseif offset == chcount - 1 then
                setStates( entity, cache, { "off", vs:sub( offset - 1, offset - 1 ), vs:sub( offset, offset ) }, color )
            elseif offset == chcount - 2 then
                setStates( entity, cache, { "off", "off", vs:sub( offset, offset ) }, color )
            elseif offset >= chcount then
                setStates( entity, cache,
                    {
                        vs:sub( offset - 2, offset - 2 ),
                        vs:sub( offset - 1, offset - 1 ),
                        vs:sub( offset, offset )
                    }, color )
            end
        end
...
local function onTickController( entity, cache )
    ...
    if selected then
        if double then
            ...
            v = entity.get_signal
            (
                selected, defines.wire_connector_id.circuit_red,
...
local function RebuildNixies()
    ...
    rendering.clear("nixie-tubes-fix")
...
remote.add_interface( "nixie-tubes-fix", { RebuildNixies = RebuildNixies } )

commands.add_command( "RebuildNixies", "Reset all Nixie Tubes to clear display glitches.", RebuildNixies )

script.on_configuration_changed( function( data )
    if data.mod_changes and data.mod_changes["nixie-tubes-fix"] then
        RebuildNixies()
    end
end )
