...
--    if storage.dispatcher == true then
--        local delivery = storage.deliveries[train.id]
--        if delivery and train.get_item_count() == 0 and train.get_fluid_count() == 0 then
--            LTN_paint(train)
--            storage.painted_ltn_trains = storage.painted_ltn_trains or {} -- move to on_init and on_configuration_changed
--            storage.painted_ltn_trains[train.id] = train -- remember each LTN train for unpainting later
--            return true
--        end
--    end
--    return false
...
--    if storage.painted_ltn_trains ~= nil then
--        local train = storage.painted_ltn_trains[train_id]
--        if not train or not train.valid then
--            storage.painted_ltn_trains[train_id] = nil
--            return
--        end
